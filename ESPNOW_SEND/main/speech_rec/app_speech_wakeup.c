...
void nnTask(void *arg)
{
	...
	while (1)
	{
		...
		if (detect_flag == 0)//如果当前不在唤醒状态下  先进行唤醒的识别
		{
			tips_wake();
		...
		else
		{
			tips_rec();
			mn_chunks++;
			int command_id = multinet->detect(model_data_mn, buffer);//对语音识别命令
...
{
	// Initialize NN model
	model_data = wakenet->create(model_coeff_getter, DET_MODE_95);//创建一个语音唤醒识别模型
	...
	model_data_mn = multinet->create(&MULTINET_COEFF, 4000);//创建一个语音命令识别模型
	// Initialize sound source
	sndQueue = xQueueCreate(5, (audio_chunksize * sizeof(int16_t)));//创建一个队列用于语音获取数据时传送给识别任务的通道
	srcif.queue = &sndQueue;
	srcif.item_size = audio_chunksize * sizeof(int16_t);

	xTaskCreatePinnedToCore(&recsrcTask, "recsrcTask", 3 * 1024, (void *)&srcif, 13, NULL, 0);//创建录音任务

	xTaskCreatePinnedToCore(&nnTask, "nnTask", 5 * 1024, NULL, 14, NULL, 0);//创建识别任务
}
